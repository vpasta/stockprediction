...
from utils.loss_functions import mse_loss, mse_loss_derivative
...
def preprocess():
    ...
    # --- Gunakan fungsi dari data_preprocessing.py untuk menghitung indikator ---
    ...
    HIDDEN_DIM = config.DEFAULT_HIDDEN_DIM # Gunakan dari config
    OUTPUT_DIM = 1
    DROPOUT_RATE = config.DEFAULT_DROPOUT_RATE # Gunakan dari config
...
def train_model():
    ticker = config.DEFAULT_TICKER # Gunakan dari config
    ...
    EPOCHS = config.DEFAULT_EPOCHS # Gunakan dari config
    LEARNING_RATE = config.DEFAULT_LEARNING_RATE # Gunakan dari config
    ...
    print(f"--- Memulai pelatihan model GRU untuk {ticker} (Epochs: {EPOCHS}, LR: {LEARNING_RATE}, Dropout: {DROPOUT_RATE}) ---")
...
def predict_price():
    ...
    gru_model.set_training_mode(False) # Set model ke mode inferensi
